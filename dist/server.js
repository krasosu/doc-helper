...
const storageMode = ((_b = process.env.STORAGE_MODE) !== null && _b !== void 0 ? _b : "s3").toLowerCase(); // "s3" | "presigned"
const s3Endpoint = (_c = process.env.MINIO_ENDPOINT) !== null && _c !== void 0 ? _c : "http://localhost:9000";
const s3Region = (_d = process.env.MINIO_REGION) !== null && _d !== void 0 ? _d : "us-east-1";
const s3Bucket = (_e = process.env.MINIO_BUCKET) !== null && _e !== void 0 ? _e : "documents";
const s3AccessKey = (_f = process.env.MINIO_ACCESS_KEY) !== null && _f !== void 0 ? _f : "minioadmin";
const s3SecretKey = (_g = process.env.MINIO_SECRET_KEY) !== null && _g !== void 0 ? _g : "minioadmin";
...
async function loadPresignedFileEntries() {
    const jsonInline = process.env.PRESIGNED_FILES_JSON;
    const jsonPath = process.env.PRESIGNED_FILES_PATH;
    if (!jsonInline && !jsonPath) {
        return [];
    }
    const raw = jsonInline !== null && jsonInline !== void 0 ? jsonInline : (await readFile(jsonPath, "utf8"));
    ...
    return list
        ...
        if (!key || !url)
            return null;
        return { key, downloadUrl: url, uploadUrl: uploadUrl !== null && uploadUrl !== void 0 ? uploadUrl : url };
    })
        .filter((x) => x !== null);
}
...
app.put(/^\/api\/static\/(.+)$/, express.raw({ type: "*/*", limit: "50mb" }), async (req, res) => {
    if (storageMode === "presigned") {
        res.status(501).json({
            ok: false,
            message: "Upload is disabled in presigned mode. Use a presigned uploadUrl from your backend.",
        });
        return;
    }
    const key = req.params[0];
